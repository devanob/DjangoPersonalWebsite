...
return input;};let porjectInsert=(projectArray)=>{console.log(projectArray);projectArray=projectArray.results.filter((element)=>{if(element.project_image!=null){return true;}})
...
document.addEventListener("DOMContentLoaded",()=>{let httpURl="/projects/api-projects/projects/?page_size=10"
fetchHTTP(httpURl).then((data)=>{porjectInsert(data);}).catch(error=>{console.log(error);})})
console.log("Testing Compression3");;var canPlay=false;var v=document.createElement('video');if(v.canPlayType&&v.canPlayType('video/mp4').replace(/no/,'')){canPlay=true;}
